...
- **`stage/*` ブランチ = project 本体の checkpoint**。main を base にした stacked 連鎖（main → stage/01 → … → 08）で、project 本体は各 stage の **`app/` 配下**に置く。`git switch stage/NN` で開ける（規約は [rules/stages.md](rules/stages.md)、設計は [docs/decisions/stage-stacked-branches.md](docs/decisions/stage-stacked-branches.md)。stage は dev flow ではなく**講義の checkpoint 機構**）
