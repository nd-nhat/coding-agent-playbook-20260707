# 決定記録: agent 自律 merge の merge gate 設計（誤 merge 防止と自律 merge の両立）

**status**: Accepted・ruleset は適用済み（required status check gate は PR-triggered CI 未充足のため未有効化、App identity gate は revert 済み — 各節参照）
**関連**: [../instructor/repo-settings.md](../instructor/repo-settings.md)（現行 ruleset の設定値と確認・変更方法）/ [app-identity-gate.md](app-identity-gate.md) / [../../rules/commit-pr.md](../../rules/commit-pr.md)（運用側の規範）
...
- それまでの間、merge gate は **review thread resolution + HOTL 判断（[../../CLAUDE.md](../../CLAUDE.md)「コミット / PR 運用」の運用規範）** で担保する。ただし **thread resolution は「agent の手の届かない server 側 barrier」ではない** — thread を resolve できるのは **PR opener または write 権限保持者**で、本 repo の agent は PR を開く張本人かつ **Pull requests: write**（[docs/guide/setup.md](../guide/setup.md)）を持つため、**bot コメントを agent 自身が resolve して gate を満たせてしまう**。つまり `required_review_thread_resolution` は機械的には merge をブロックするが、agent に対する強制力は無く、**監査可能な workflow ステップ（誰がどう対応したかが thread に残る）**として機能する。CI gate が未充足な現状、agent に対する実質的な抑止は **HOTL 判断**（agent はデフォルトで merge せず報告して停止し、merge 実行はユーザー判断。[../../CLAUDE.md](../../CLAUDE.md) Step 6）である点を明確にしておく。
...
原則 1 の「human approval を gate にしない」は **author=agent=owner (PAT) だと自己 approve 不可で詰む**ことから導いた。**box を GitHub App bot identity で回す**と author=bot になり、**human owner（別 identity）が approve できる** → 原則 1 の「詰む」が解け、`required_approving_review_count: 1` が **satisfiable かつ機械強制な gate** になる。設計・機構・実機検証は [app-identity-gate.md](app-identity-gate.md)、有効化手順は [docs/guide/setup.md](../guide/setup.md)「github を App identity 化する」参照。
...
- **実運用検証は完了**: `0→1` の flip 自体は App 発 PR を実運用で human approve できることを確認済み（[app-identity-gate.md](app-identity-gate.md) 「残差・未決」参照）。
