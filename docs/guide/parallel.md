...
起票そのものは人が `gh` を手打ちするより **claude に頼んで作る**のが通常（box から起票するには PAT に `Issues: Read and write` が要る。[docs/guide/setup.md](setup.md)。無いと `Resource not accessible by personal access token (createIssue)`）。違いは「**何を**起票するか」の source:
...
> - 対象 checkpoint 名は実ブランチと `docs/instructor/README.md` の checkpoint 表に合わせる。
