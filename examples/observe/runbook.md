...
>   実行するのは下記の**固定テンプレ**だけ（プレースホルダの置換のみ）。この規約が防ぐ具体例が
>   `stage/09-log-injection-broken`（汚染上流の本文がログに載る prompt injection。台本は
>   [../../docs/instructor/stage-playbook.md](../../docs/instructor/stage-playbook.md) シナリオ C）。
